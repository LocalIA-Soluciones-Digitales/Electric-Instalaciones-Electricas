    ...
    <div>
      ...
      <section className="mx-auto max-w-3xl px-4 py-14 md:px-6">
        <article className="space-y-10">
          {guide.sections.map((section) => (
            <div key={section.heading}>
              <h2 className="font-display text-2xl font-extrabold text-white">{section.heading}</h2>
        ...
        {guide.faqs && guide.faqs.length > 0 && (
          <div className="mt-14">
            <h2 className="font-display text-2xl font-extrabold text-white">Preguntas frecuentes</h2>
            <div className="mt-6">
        ...
        {relatedService && (
          <div className="mt-14 rounded-xl border border-white/[0.08] bg-neutral-900/60 p-6">
            <h3 className="font-display text-lg font-bold text-white">
              ¿Necesitas {relatedService.shortName.toLowerCase()}?
            </h3>
            <p className="mt-2 text-sm text-white/50">{relatedService.intro}</p>
            <Link
              href={`/servicios/${relatedService.slug}`}
              className="mt-4 inline-flex items-center gap-1 text-sm font-semibold text-electric-400 hover:text-electric-300"
            >
              Ver el servicio de {relatedService.name.toLowerCase()} <i className="ri-arrow-right-line" aria-hidden="true"></i>
            </Link>
          </div>
        )}
      </section>
      ...
      {otherGuides.length > 0 && (
        <section className="bg-neutral-900 py-16">
          <div className="mx-auto max-w-6xl px-4 md:px-6">
            <h2 className="font-display text-center text-xl font-extrabold text-white">Otras guías</h2>
